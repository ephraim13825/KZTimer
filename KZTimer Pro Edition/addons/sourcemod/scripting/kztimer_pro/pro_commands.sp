...
/*public Action:Client_Test(client, args)
{
	for (new i; i < GetEntityCount(); i++)
    {
        if (IsValidEdict(i))
		{
			decl String:targetname[64];
			GetEntPropString(i, Prop_Data, "m_iName", targetname, sizeof(targetname));
			PrintToConsole(client,"targetname: %s", targetname);
		}	
	}
	return Plugin_Handled;
}*/
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsValidEntity(i) && IsClientInGame(i) && IsPlayerAlive(i) && i != client && g_bChallengeRequest[i])
		{
			...
			{		
				...
				TeleportEntity(client, g_fCStartPosition[i],NULL_VECTOR, Float:{0.0,0.0,-100.0});
				TeleportEntity(i, g_fCStartPosition[i],NULL_VECTOR, Float:{0.0,0.0,-100.0});
...
public Action:Client_Prestrafewarning(client, args) 
{
	PrintToChat(client, "disabled.");
	/*prestrafewarning(client);
	if (g_bPreStrafeWarning[client] == true)
		PrintToChat(client, "%t", "PreWarning1",MOSSGREEN, WHITE);
	else
		PrintToChat(client, "%t", "PreWarning2",MOSSGREEN, WHITE);*/
	return Plugin_Handled;
}


public prestrafewarning(client)
{
	if (g_bPreStrafeWarning[client] == true)
		g_bPreStrafeWarning[client] = false;
	else
		g_bPreStrafeWarning[client] = true;
}
...
public MapTopMenu(client)
{
	...
	if (!g_bMapButtons && g_bGlobalDB && g_BGlobalDBConnected && g_hDbGlobal != INVALID_HANDLE && g_bglobalValidFilesize && g_bAntiCheat && !g_bAutoBhop2)
	{
...
public MapTopMenuHandler(Handle:menu, MenuAction:action, param1,param2)
{
	if(action == MenuAction_Select)
	{
		if (!g_bMapButtons && g_bGlobalDB && g_BGlobalDBConnected && g_hDbGlobal != INVALID_HANDLE && g_bglobalValidFilesize && g_bAntiCheat && !g_bAutoBhop2)
		{
			switch(param2)
			{
				case 0: db_selectGlobalTopClimbers102(param1);
				case 1: db_selectTopClimbers(param1,g_szMapName);
				case 2: db_selectProClimbers(param1);
				case 3: db_selectTPClimbers(param1);
			}
		}
		else
		{
...
public JumpTopMenuHandler(Handle:menu, MenuAction:action, param1,param2)
{
	if(action == MenuAction_Select)
	{
		switch(param2)
		{
			...
			case 2: db_selectTopDropBhop(param1);
			case 3: db_selectTopWj(param1);
		}
	}
	else
	if(action == MenuAction_Cancel)
...
public OptionMenu(client)
{
	...
	//11
	if (g_bPreStrafeWarning[client])
		AddMenuItem(optionmenu, "Goto  -  Enabled", "Prestrafe warning  -  Enabled",ITEMDRAW_DISABLED);
	else
		AddMenuItem(optionmenu, "Goto  -  Disabled", "Prestrafe warning  -  Disabled",ITEMDRAW_DISABLED);					
...
public OptionMenuHandler(Handle:menu, MenuAction:action, param1,param2)
{
	if(action == MenuAction_Select)
	{
		...
		{	
			...
			case 12: prestrafewarning(param1);
		}
