...
new String:sqlglobal_insertBan[] 				= "INSERT INTO banlistpro (steamid, player) VALUES('%s', '%s');";
new String:sqlglobal_deleteban3[] 				= "DELETE FROM player102pro WHERE steamid = '%s'"; 
new String:sqlglobal_selectGlobalRecord102[] 	= "SELECT name, runtime, teleports FROM player102pro WHERE mapname = '%s' ORDER BY runtime ASC LIMIT 5";
new String:sqlglobal_selectGlobalTop102[] 		= "SELECT name, runtime, teleports FROM player102pro WHERE mapname = '%s' ORDER BY runtime ASC LIMIT 5";
new String:sqlglobal_selectTop5Players102[] 	= "SELECT runtime, steamid FROM player102pro WHERE mapname = '%s' ORDER BY runtime ASC LIMIT 5";
new String:sqlglobal_selectPlayers102[] 		= "SELECT runtime, steamid, mapname FROM player102pro WHERE mapname = '%s' ORDER BY runtime ASC";
new String:sqlglobal_deletePlayer102[] 			= "DELETE FROM player102pro WHERE steamid = '%s' AND mapname = '%s'"; 
new String:sqlglobal_insertPlayer102[] 			= "INSERT INTO player102pro (steamid, mapname, name, runtime, teleports) VALUES('%s', '%s', '%s', '%f', '%i');";
new String:sqlglobal_updatePlayer102[] 			= "UPDATE player102pro SET name = '%s', runtime = '%f', teleports = '%i' WHERE steamid = '%s' AND mapname = '%s';";
// ADMIN 
...
new String:sqlite_dropPlayerJump[] 			= "DROP TABLE playerjumpstats2; VACCUM";
new String:sql_dropPlayerJump[] 				= "DROP TABLE playerjumpstats2;";
...
new String:sql_resetBhopRecord[] 			= "UPDATE playerjumpstats2 SET bhoprecord = '-1.0' WHERE steamid = '%s';";   
new String:sql_resetDropBhopRecord[] 		= "UPDATE playerjumpstats2 SET dropbhoprecord = '-1.0' WHERE steamid = '%s';";   
new String:sql_resetWJRecord[] 				= "UPDATE playerjumpstats2 SET wjrecord = '-1.0' WHERE steamid = '%s';";   
new String:sql_resetLjRecord[] 				= "UPDATE playerjumpstats2 SET ljrecord = '-1.0' WHERE steamid = '%s';";  
new String:sql_resetMultiBhopRecord[] 		= "UPDATE playerjumpstats2 SET multibhoprecord = '-1.0' WHERE steamid = '%s';";  
new String:sql_resetJumpStats[] 				= "UPDATE playerjumpstats2 SET multibhoprecord = '-1.0', ljrecord = '-1.0', wjrecord = '-1.0', dropbhoprecord = '-1.0', bhoprecord = '-1.0' WHERE steamid = '%s';";  
...
{   
	...
	if (hndl != INVALID_HANDLE)
		...
		{		
			...
			SetMenuTitle(menu, "Top 5 Global Climbers (tickrate 102.4)\n       Time            TP's        Player");    
			new i=1;
			while (SQL_FetchRow(hndl))
			{
...
public db_setupDatabase()
{
	...
	g_hDb = SQL_Connect("kztimerpro", false, szError, 255);
...
public SQL_ViewJumpStatsCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if(SQL_HasResultSet(hndl) && SQL_FetchRow(hndl))
	{
		...
		if (bhoprecord >0.0 || ljrecord > 0.0 || wjrecord > 0.0 || dropbhoprecord > 0.0)
		{										 
...
public sql_selectLatestRecordsCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	new Float: ftime;
	PrintToConsole(client, "----------------------------------------------------------------------------------------------------");
	PrintToConsole(client," KZTimer Pro - latest records:");
	if(SQL_HasResultSet(hndl))
	{		
		new i = 1;
		while (SQL_FetchRow(hndl))
...
{       
	...
	Format(title, 256, "Top 50 Climbers on %s (local)\n    Rank    Time          TP's        Player", szMap);
...
public db_viewPlayerOptionsCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	{		
		...
		g_bPreStrafeWarning[client]=IntoBool(SQL_FetchInt(hndl, 15));
		//org
		...
		g_borg_PreStrafeWarning[client] = g_bPreStrafeWarning[client];
	}
	else
	{
		decl String:szQuery[512];      
