...
public KzAdminMenu(client)
{
	...
	AddMenuItem(adminmenu, szTmp, szTmp);	
	if (g_bGlobalDB)
		Format(szTmp, sizeof(szTmp), "[16.] Global database  -  Enabled"); 	
	else
		Format(szTmp, sizeof(szTmp), "[16.] Global database  -  Disabled"); 	
	AddMenuItem(adminmenu, szTmp, szTmp);	
	if (g_bReplayBot)
	...
	if (g_bPlayerSkinChange)
		Format(szTmp, sizeof(szTmp), "[20.] Allows custom models -  Enabled"); 	
	else
		Format(szTmp, sizeof(szTmp), "[20.] Allows custom models  -  Disabled"); 				
	AddMenuItem(adminmenu, szTmp, szTmp);	
	if (g_bNoClipS)
		Format(szTmp, sizeof(szTmp), "[21.] No clip for players -  Enabled"); 	
	else
		Format(szTmp, sizeof(szTmp), "[21.] No clip for players  -  Disabled"); 				
	AddMenuItem(adminmenu, szTmp, szTmp);	
	if (g_bJumpStats)
	...
	AddMenuItem(adminmenu, szTmp, szTmp);
	if (g_bAutoBan)
		Format(szTmp, sizeof(szTmp), "[23.] AntiCheat auto-ban -  Enabled"); 	
	else
		Format(szTmp, sizeof(szTmp), "[23.] AntiCheat auto-ban  -  Disabled"); 			
	AddMenuItem(adminmenu, szTmp, szTmp);
	if (g_bMultiplayerBhop)
		Format(szTmp, sizeof(szTmp), "[24.] Multiplayer bunnyhop -  Enabled"); 	
	else
		Format(szTmp, sizeof(szTmp), "[24.] Multiplayer bunnyhop  -  Disabled"); 			
	AddMenuItem(adminmenu, szTmp, szTmp);
	if (g_bAdminClantag)
		Format(szTmp, sizeof(szTmp), "[25.] Player admin tag -  Enabled"); 	
	else
		Format(szTmp, sizeof(szTmp), "[25.] Player admin tag  -  Disabled"); 			
	AddMenuItem(adminmenu, szTmp, szTmp);	
	if (g_bVipClantag)
		Format(szTmp, sizeof(szTmp), "[26.] Player VIP tag -  Enabled"); 	
	else
		Format(szTmp, sizeof(szTmp), "[26.] Player VIP tag  -  Disabled"); 		
	AddMenuItem(adminmenu, szTmp, szTmp);		
	if (g_bMapEnd)
		Format(szTmp, sizeof(szTmp), "[27.] Map end -  Enabled"); 	
	else
		Format(szTmp, sizeof(szTmp), "[27.] Map end  -  Disabled"); 		
	AddMenuItem(adminmenu, szTmp, szTmp);
	if (g_bConnectMsg)
...
public Action:Admin_ResetAllLjRecords(client, args)
{
	...
	Format(szQuery, 255, "UPDATE playerjumpstats2 SET ljrecord=-1.0");
...
{
	decl String:szQuery[255];      
	Format(szQuery, 255, "UPDATE playerjumpstats2 SET wjrecord=-1.0");
	SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery);	       
...
{
	decl String:szQuery[255];      
	Format(szQuery, 255, "UPDATE playerjumpstats2 SET bhoprecord=-1.0");
	SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery);	       
...
{
	decl String:szQuery[255];      
	Format(szQuery, 255, "UPDATE playerjumpstats2 SET dropbhoprecord=-1.0");
	SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery);	
...
{
	decl String:szQuery[255];      
	Format(szQuery, 255, "UPDATE playerjumpstats2 SET multibhoprecord=-1.0");
	SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery);	
