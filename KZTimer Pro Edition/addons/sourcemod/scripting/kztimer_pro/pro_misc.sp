...
public SetServerTags()
{
	...
	Format(szServerTags, 1024, "%s, kztimer pro edition, tickrate %i",szServerTags,g_tickrate);
...
public PrintConsoleInfo(client)
{
	...
	PrintToConsole(client, "!help, !menu, !options, !checkpoint, !gocheck, !prev, !next, !undo, !profile, !compare, !bhopcheck <name>,");
	PrintToConsole(client, "!maptop <map>, !top, !start, !stop, !pause, !usp, !challenge, !surrender, !goto, !spec, !showsettings,");
	PrintToConsole(client, "!latest, !measure");
	PrintToConsole(client, "Options menu contains: !adv, !info, !colorchat, !cpmessage, !sound, !menusound, !hide, !hidespecs,");
	PrintToConsole(client, "!showtime, !disablegoto, !shownames, !sync)");
	PrintToConsole(client, " ");
...
public PrintMapRecords(client)
{
	if (g_fRecordTimeGlobal102 != 9999999.0)
	{
		...
		PrintToChat(client, "[%cKZ%c] %cGLOBAL RECORD%c: %s (%s)",MOSSGREEN,WHITE,RED,WHITE, g_szTime[client], g_szRecordGlobalPlayer102); 
	}	
...
public SetSkillGroups()
{
	...
	Format(sPath, sizeof(sPath), "configs/kztimer_pro/skill_groups.txt");
