...
public CL_OnEndTimerPress(client)
{
	...
	//NEW GLOBAL RECORD 
	if (!g_bMapButtons && g_bAntiCheat && g_bglobalValidFilesize && g_BGlobalDBConnected && g_hDbGlobal != INVALID_HANDLE && g_bEnforcer) 
	{
		if (g_fFinalTime[client] < g_fRecordTimeGlobal102 && g_fFinalTime[client] > 20.0)
		{
	...
	//global db update
	if (!g_bMapButtons && g_bGlobalDB && g_BGlobalDBConnected && g_hDbGlobal != INVALID_HANDLE && g_bEnforcer && g_bAntiCheat && g_bglobalValidFilesize)	
