...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon, &subtype, &cmdnum, &tickcount, &seed, mouse[2])
{
	...
	{	
		...
		/*if (g_ground_frames[client] >= 12 && g_bPrestrafeTooHigh[client])
			...
			if ((aiPatternhits[client] > 0) && (aiJumps[client] == aiPattern[client]))
			{
				...
				if (aiPatternhits[client] > 15)
				{
					if (g_bAntiCheat && !bFlagged[client])
					{
						...
						BuildPath(Path_SM, sPath, sizeof(sPath), "logs/kztimerpro_anticheat.log");
...
{	
	...
	{				
		//block invalid bot distances (has something to do with the ground-detection of the replay bot) && strafe hack block
	...
	//BunnyHop
	if ((!g_bLadderJump[client] && ground_frames < 11 && g_last_ground_frames[client] > 10 && fGroundDiff == 0.0 && fJump_Height <= 66.0 && !g_bDropJump[client] && g_fPreStrafe[client] > 200.0) || (!g_bLadderJump[client] && g_last_ground_frames[client] < 11 && ground_frames < 11 && fGroundDiff == 0.0  && fJump_Height <= 66.0 && !g_bDropJump[client]))
	{
			//block invalid bot distances (has something to do with the ground-detection of the replay bot) WORKAROUND
