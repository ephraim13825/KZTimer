...
#define VERSION "1.32 Pro Edition"
...
enum JumpType (+=1)
{
	...
	JumpType_WeirdJump,
	JumpType_BhopJump,
	JumpType_MultiBhopJump,
	JumpType_DropBhopJump,
}
...
new bool:g_bPreStrafeWarning[MAXPLAYERS+1]=false;
//org
...
new bool:g_borg_PreStrafeWarning[MAXPLAYERS+1];
...
new String:BlockedChatText[][] = {"!help","!usp","!helpmenu","!menu","!menu ","!checkpoint","!gocheck","!unstuck",
	...
	"!knife","!adv", "!info", "!colorchat", "!cpmessage", "!sound", "!menusound", "!hide", "!hidespecs", "!showtime", "!disablegoto", "!shownames", "!sync", "!speed", "!showkeys", "!goto", "!profile", "!measure", "!rank", "!prewarning",
	"/knife","/adv", "/info", "/colorchat", "/cpmessage", "/sound", "/menusound", "/hide", "/hidespecs", "/showtime", "/disablegoto", "/shownames", "/sync", "/speed", "/showkeys", "/goto", "/profile", "/measure", "/rank", "/prewarning"};
...
#include "kztimer_pro/pro_admin.sp"
#include "kztimer_pro/pro_commands.sp"
#include "kztimer_pro/pro_hooks.sp"
#include "kztimer_pro/pro_buttonpress.sp"
#include "kztimer_pro/pro_sql.sp"
#include "kztimer_pro/pro_misc.sp"
#include "kztimer_pro/pro_timer.sp"
#include "kztimer_pro/pro_replay.sp"
#include "kztimer_pro/pro_globalconnection.sp"
	
public Plugin:
myinfo = {
	name = "KZTimer Pro Edition",
	author = "1NutWunDeR",
	description = "",
	version = VERSION,
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	CreateNative("KZTimerPro_GetTimerStatus", Native_GetTimerStatus);
	CreateNative("KZTimerPro_StopUpdatingOfClimbersMenu", Native_StopUpdatingOfClimbersMenu);
	CreateNative("KZTimerPro_StopTimer", Native_StopTimer);
	g_bLateLoaded = late;
...
public OnPluginStart()
{
	...
	LoadTranslations("kztimerpro.phrases");	
	CreateConVar("kztimerpro_version", VERSION, "kztimer pro Version.", FCVAR_DONTRECORD|FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	...
	if (g_tickrate != 102)		
	{
		SetFailState("[KZPro] Only tickrate 102.4 supported. Plugin paused!");
	}
	...
	g_hWelcomeMsg   = CreateConVar("kz_welcome_msg", "Welcome. This server is using KZ Timer Pro Edition","Welcome message", FCVAR_PLUGIN|FCVAR_NOTIFY);
	GetConVarString(g_hWelcomeMsg,g_sWelcomeMsg,512);
	HookConVarChange(g_hWelcomeMsg, OnSettingChanged);
	
	g_hReplayBotProColor   = CreateConVar("kz_replay_bot_pro_color", "0 0 255","Pro replay bot color. Format: \"red green blue\" from 0 - 255.", FCVAR_PLUGIN|FCVAR_NOTIFY);
	...
	g_hReplayBotTpColor   = CreateConVar("kz_replay_bot_tp_color", "255 127 59","Tp replay bot color. Format: \"red green blue\" from 0 - 255.", FCVAR_PLUGIN|FCVAR_NOTIFY);
	...
	g_hAutoBan 	= CreateConVar("kz_anticheat_auto_ban", "0", "on/off - auto-ban (bhop hacks and strafe hacks) including deletion of all player records - Info: There's always an anticheat log (sourcemod/logs) even if this function is disabled", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	g_hdist_good_lj    	= CreateConVar("kz_dist_min_lj", "225.0", "Minimum distance for longjumps to be considered good [Client Message]", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 200.0, true, 999.0);
	g_hdist_pro_lj   	= CreateConVar("kz_dist_pro_lj", "245.0", "Minimum distance for longjumps to be considered pro [JumpStats Colorchat All]", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 200.0, true, 999.0);
	g_hdist_leet_lj    	= CreateConVar("kz_dist_leet_lj", "252.0", "Minimum distance for longjumps to be considered leet [JumpStats Colorchat All]", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 200.0, true, 999.0);		
	...
	g_hdist_pro_weird  = CreateConVar("kz_dist_pro_wj", "255.0", "Minimum distance for weird jumps to be considered pro [JumpStats Colorchat All]", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 200.0, true, 999.0);
	g_hdist_leet_weird   = CreateConVar("kz_dist_leet_wj", "260.0", "Minimum distance for weird jumps to be considered leet [JumpStats Colorchat All]", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 200.0, true, 999.0);
	g_hdist_good_dropbhop  = CreateConVar("kz_dist_min_dropbhop", "220.0", "Minimum distance for drop bhops to be considered good [Client Message]", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 200.0, true, 999.0);
	g_hdist_pro_dropbhop  = CreateConVar("kz_dist_pro_dropbhop", "258.0", "Minimum distance for drop bhops to be considered pro [JumpStats Colorchat All]", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 200.0, true, 999.0);
	g_hdist_leet_dropbhop   = CreateConVar("kz_dist_leet_dropbhop", "263.0", "Minimum distance for drop bhops to be considered leet [JumpStats Colorchat All]", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 200.0, true, 999.0);
	g_hdist_good_bhop  = CreateConVar("kz_dist_min_bhop", "220.0", "Minimum distance for bhops to be considered good [Client Message]", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 200.0, true, 999.0);
	g_hdist_pro_bhop  = CreateConVar("kz_dist_pro_bhop", "255.0", "Minimum distance for bhops to be considered pro [JumpStats Colorchat All]", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 200.0, true, 999.0);
	g_hdist_leet_bhop   = CreateConVar("kz_dist_leet_bhop", "260.0", "Minimum distance for bhops to be considered leet [JumpStats Colorchat All]", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 200.0, true, 999.0);
	...
	//client commands
	RegConsoleCmd("sm_accept", Client_Accept, "[KZTimer Pro] allows you to accept a challenge request");
	RegConsoleCmd("sm_goto", Client_GoTo, "[KZTimer Pro] teleports you to a selected player");
	RegConsoleCmd("sm_disablegoto", Client_DisableGoTo, "[KZTimer Pro] on/off speed/showkeys center panel");
	RegConsoleCmd("sm_showkeys", Client_InfoPanel, "[KZTimer Pro] on/off speed/showkeys center panel");
	RegConsoleCmd("sm_info", Client_InfoPanel, "[KZTimer Pro] on/off speed/showkeys center panel");
	RegConsoleCmd("sm_menusound", Client_ClimbersMenuSounds,"[KZTimer Pro] on/off climbers menu sounds");
	RegConsoleCmd("sm_sync", Client_StrafeSync,"[KZTimer Pro] on/off strafe sync in chat");
	RegConsoleCmd("sm_sound", Client_QuakeSounds,"[KZTimer Pro] on/off quake sounds");
	RegConsoleCmd("sm_cpmessage", Client_CPMessage,"[KZTimer Pro] on/off checkpoint message in chat");
	RegConsoleCmd("sm_surrender", Client_Surrender, "[KZTimer Pro] surrender your current challenge");
	RegConsoleCmd("sm_next", Client_Next,"[KZTimer Pro] goto next checkpoint");
	RegConsoleCmd("sm_usp", Client_Usp, "[KZTimer Pro] spawns a usp silencer");
	RegConsoleCmd("sm_bhop", Client_AutoBhop,"[KZTimer Pro] on/off autobhop (only mg_,surf_ and bhop_ maps supported)");
	RegConsoleCmd("sm_undo", Client_Undo,"[KZTimer Pro] undoes your last telepoint");
	RegConsoleCmd("sm_prewarning", Client_Prestrafewarning,"[KZTimer Pro] Prestrafe warning message");
	RegConsoleCmd("sm_prev", Client_Prev,"[KZTimer Pro] goto previous checkpoint");
	RegConsoleCmd("sm_adv", Client_AdvClimbersMenu, "[KZTimer Pro] advanced climbers menu (additional: !next, !prev and !undo)");
	RegConsoleCmd("sm_unstuck", Client_Prev,"[KZTimer Pro] go to previous checkpoint");
	RegConsoleCmd("sm_maptop", Client_MapTop,"[KZTimer Pro] displays local map top for a given map");
	RegConsoleCmd("sm_stuck", Client_Prev,"[KZTimer Pro] go to previous checkpoint");
	...
	RegConsoleCmd("sm_challenge", Client_Challenge, "[KZTimer Pro] starts a map race against someone");
	RegConsoleCmd("sm_helpmenu", Client_Help, "[KZTimer Pro] help menu which displays all kztimer commands");
	RegConsoleCmd("sm_help", Client_Help, "[KZTimer Pro] help menu which displays all kztimer commands");
	RegConsoleCmd("sm_profile", Client_Profile, "[KZTimer Pro] opens a player profile");
	RegConsoleCmd("sm_rank", Client_Profile, "[KZTimer Pro] opens a player profile");
	RegConsoleCmd("sm_options", Client_OptionMenu, "[KZTimer Pro] opens options menu");
	RegConsoleCmd("sm_top", Client_Top, "[KZTimer Pro] displays top rankings (Top 100 Players, Top 5 Global, Top 5 Global 128tick, Top50 overall, Top 20 Pro, Top 20 with Teleports, Top 20 LJ, Top 20 Bhop, Top 20 WeirdJump, Top 20 Drop Bunnyhop)");
	RegConsoleCmd("sm_topclimbers", Client_Top, "[KZTimer Pro] displays top rankings (Top 100 Players, Top 5 Global, Top 5 Global 128tick, Top50 overall, Top 20 Pro, Top 20 with Teleports, Top 20 LJ, Top 20 Bhop, Top 20 WeirdJump, Top 20 Drop Bunnyhop)");
	RegConsoleCmd("sm_top15", Client_Top, "[KZTimer Pro] displays top rankings (Top 100 Players, Top 5 Global, Top 5 Global 128tick, Top50 overall, Top 20 Pro, Top 20 with Teleports, Top 20 LJ, Top 20 Bhop, Top 20 WeirdJump, Top 20 Drop Bunnyhop)");
	RegConsoleCmd("sm_start", Client_Start, "[KZTimer Pro] starts the map timer");
	RegConsoleCmd("sm_r", Client_Start, "[KZTimer Pro] starts the map timer");
	RegConsoleCmd("sm_stop", Client_Stop, "[KZTimer Pro] stops the map timer");
	RegConsoleCmd("sm_speed", Client_InfoPanel, "[KZTimer Pro] on/off speed/showkeys center panel");
	RegConsoleCmd("sm_pause", Client_Pause,"[KZTimer Pro] on/off client pause (sets your timer on hold and freezes your current position)");
	RegConsoleCmd("sm_colorchat", Client_Colorchat, "[KZTimer Pro] on/off jumpstats messages of others in chat");
	RegConsoleCmd("sm_showsettings", Client_Showsettings,"[KZTimer Pro] shows server settings");
	RegConsoleCmd("sm_latest", Client_Latest,"[KZTimer Pro] shows latest map records");
	RegConsoleCmd("sm_showtime", Client_Showtime,"[KZTimer Pro] on/off - timer text in panel/menu");	
	RegConsoleCmd("sm_shownames", Client_Shownames, "[KZTimer Pro] on/off target name center panel");
	RegConsoleCmd("sm_hide", Client_Hide, "[KZTimer Pro] on/off - hides other players"); 
	RegConsoleCmd("+noclip", NoClip, "[KZTimer Pro] Player noclip on");
	RegConsoleCmd("-noclip", UnNoClip, "[KZTimer Pro] Player noclip off");
	RegConsoleCmd("sm_bhopcheck", Command_Stats, "[KZTimer Pro] check bhop stats for a given player");
	RegAdminCmd("sm_kzadmin", Admin_KzPanel, ADMIN_LEVEL, "[KZTimer Pro] Displays the kz admin panel");
	RegAdminCmd("sm_resettimes", Admin_DropAllMapRecords, ADMIN_LEVEL, "[KZTimer Pro] Resets player times (drops table playertimes)");
	RegAdminCmd("sm_resetranks", Admin_DropPlayerRanks, ADMIN_LEVEL, "[KZTimer Pro] Resets the player point system (drops table playerrank)");
	RegAdminCmd("sm_resetmaptimes", Admin_ResetMapRecords, ADMIN_LEVEL, "[KZTimer Pro] Resets player times for given map");
	RegAdminCmd("sm_resetplayertimes", Admin_ResetRecords, ADMIN_LEVEL, "[KZTimer Pro] Resets tp+pro map times for given steamid with or without given map");	
	RegAdminCmd("sm_resetplayertptime", Admin_ResetRecordTp, ADMIN_LEVEL, "[KZTimer Pro] Resets tp map time for given steamid and map");
	RegAdminCmd("sm_resetplayerprotime", Admin_ResetRecordPro, ADMIN_LEVEL, "[KZTimer Pro] Resets pro map time for given steamid and map");
	RegAdminCmd("sm_resetjumpstats", Admin_DropPlayerJump, ADMIN_LEVEL, "[KZTimer Pro] Resets all jump stats (drops table playerjumpstats)");	
	RegAdminCmd("sm_resetallljrecords", Admin_ResetAllLjRecords, ADMIN_LEVEL, "[KZTimer Pro] Resets all lj records");
	RegAdminCmd("sm_resetallwjrecords", Admin_ResetAllWjRecords, ADMIN_LEVEL, "[KZTimer Pro] Resets all wj records");
	RegAdminCmd("sm_resetallbhoprecords", Admin_ResetAllBhopRecords, ADMIN_LEVEL, "[KZTimer Pro] Resets all bhop records");
	RegAdminCmd("sm_resetalldropbhopecords", Admin_ResetAllDropBhopRecords, ADMIN_LEVEL, "[KZTimer Pro] Resets all drop bjop records");
	RegAdminCmd("sm_resetljrecord", Admin_ResetLjRecords, ADMIN_LEVEL, "[KZTimer Pro] Resets lj record for given steamid");
	RegAdminCmd("sm_resetbhoprecord", Admin_ResetBhopRecords, ADMIN_LEVEL, "[KZTimer Pro] Resets bhop record for given steamid");	
	RegAdminCmd("sm_resetdropbhoprecord", Admin_ResetDropBhopRecords, ADMIN_LEVEL, "[KZTimer Pro] Resets drop bhop record for given steamid");
	RegAdminCmd("sm_resetwjrecord", Admin_ResetWjRecords, ADMIN_LEVEL, "[KZTimer Pro] Resets wj record for given steamid");	
	RegAdminCmd("sm_resetplayerjumpstats", Admin_ResetPlayerJumpstats, ADMIN_LEVEL, "[KZTimer] Resets jump stats for given steamid");
	RegAdminCmd("sm_deleteproreplay", Admin_DeleteProReplay, ADMIN_LEVEL, "[KZTimer Pro] Deletes pro replay for a given map");
	RegAdminCmd("sm_deletetpreplay", Admin_DeleteTpReplay, ADMIN_LEVEL, "[KZTimer Pro] Deletes tp replay for a given map");	
	RegAdminCmd("sm_getmultiplier", Admin_GetMulitplier, ADMIN_LEVEL, "[KZTimer Pro] Gets the dynamic multiplier for given player (points)");
	RegAdminCmd("sm_setmultiplier", Admin_SetMulitplier, ADMIN_LEVEL, "[KZTimer Pro] Sets the dynamic multiplier for given player and mutliplier value (points)");	
	RegConsoleCmd("say", Say_Hook);
	RegConsoleCmd("say_team", Say_Hook);
	AutoExecConfig(true, "kztimerpro");
...
public OnMapStart()
{
	...
	InitPrecache();	
	SetCashState();
	CreateTimer(0.1, SpeedTimer, _, TIMER_FLAG_NO_MAPCHANGE|TIMER_REPEAT);//test.. 
	...
	ServerCommand("sv_max_queries_sec 6");	
...
{	
	...
	g_bPreStrafeWarning[client]=false;
